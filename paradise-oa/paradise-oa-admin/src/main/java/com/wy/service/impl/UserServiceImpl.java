...
public class UserServiceImpl extends AbstractService<User, Long> implements UserService {
	...
	// @Autowired
	// private FileAllService fileService;
	//
	// @Autowired
	...
	// UserOld user = userDao.getById(userId);
	// if (StrUtils.isNotBlank(user.getUserphoto())) {
	// boolean flag = fileService.delByLocalName(user.getUserphoto());
	// if (!flag) {
	// log.info("删除用户本地头像失败!图片本地名:{0},用户userId:{1}", user.getUserphoto(), userId);
	// }
	// }
	// FileAll relatedFile = fileService.uploadFile(file);
	// return userDao.update(MapUtils.getBuilder("user_icon",
	// relatedFile.getLocalName()).build(),
	...
	// processEngine.getRuntimeService().startProcessInstanceByKey("leave", params);
	...
	// List<Task> tasks =
	// taskService.createTaskQuery().taskAssignee("12345").list();
	// for (Task task : tasks) {
