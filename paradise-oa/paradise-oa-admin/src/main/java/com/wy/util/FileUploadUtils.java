...
public class FileUploadUtils {
	...
	public static final String upload(String baseDir, MultipartFile file, String[] allowedExtension) {
		...
		if (fileNamelength > FileUploadUtils.DEFAULT_FILE_NAME_LENGTH) {
			throw new ResultException("文件名最大长度为" + FileUploadUtils.DEFAULT_FILE_NAME_LENGTH);
		}
	...
	public static final String extractFilename(MultipartFile file) {
		...
		fileName = DateUtils.format(new Date(), DateEnum.DATE_SLASH) + File.separator + encodingFilename(fileName) + "."
				+ extension;
		return fileName;
	}
	...
	private static final String encodingFilename(String fileName) {
		...
		fileName = CryptoUtils.MD5(fileName + System.nanoTime() + counter++);
		return fileName;
	}
	...
	public static final void assertAllowed(MultipartFile file, String[] allowedExtension) {
		...
		if (DEFAULT_MAX_SIZE != -1 && size > DEFAULT_MAX_SIZE) {
			throw new ResultException("文件最大为" + DEFAULT_MAX_SIZE / 1024 / 1024);
		}
		...
		if (allowedExtension != null && !isAllowedExtension(extension, allowedExtension)) {
			if (allowedExtension == MimeTypeUtils.IMAGE_EXTENSION) {
				throw new ResultException(fileName + "文件格式错误");
